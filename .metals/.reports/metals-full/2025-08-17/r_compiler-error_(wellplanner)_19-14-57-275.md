...
### scala.reflect.internal.FatalError: 
  ...
  last tree to typer: ApplyToImplicitArgs(method toJsFieldJsValueWrapper)
       tree position: line 286 of file://<WORKSPACE>/app/services/PaymentService.scala
...
== Source file context for tree position ==
   ...
   283             _CURSOR_stripePaymentIntentId = paymentIntentId,
   284             status = PaymentIntentStatus.RequiresPaymentMethod
   285           )
   286         } yield Json.obj("status" -> "processed", "event" -> eventType)
   287 
   288       case _ =>
   289         Future.successful(Json.obj("status" -> "ignored", "event" -> eventType))
